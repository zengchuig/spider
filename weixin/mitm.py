...
class A:
	def request(self,flow):
		if article_list in flow.request.url:
			...
			sql='insert into req(params,cookie,appmsg_token) values("{}","","");'.format(self.params)
			try:
				cs.execute(sql)
				conn.commit()
				cs.close()
				...
				conn.close()
			print('--------------------------------')
			# self.cookie=flow.request.headers['Cookie']
			# self.cookie={i.split('=',1)[0].strip():i.split('=',1)[1] for i in self.cookie.split(';')}
	...
	# 		print(appmsg_token)
	# 		sql='insert into req(params,cookie,appmsg_token) values("{}","{}","{}");'.format(self.params,self.st[:-1],appmsg_token)
	# 		try:
	# 			cs.execute(sql)
	# 			conn.commit()
	# 			cs.close()
